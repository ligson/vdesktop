...
public class Demo2 {
	public static void main(String[] args) throws Exception {
		...
		DoubleIndexer Ridx = randomR.createIndexer(), axisIdx = randomAxis
				.createIndexer();
		axisIdx.put(0, (Math.random() - 0.5) / 4, (Math.random() - 0.5) / 4,
				(Math.random() - 0.5) / 4);
		...
		while (frame.isVisible()
			...
			for (int i = 0; i < total; i++) {
				...
				cvFillConvexPoly(grabbedImage, hatPoints.position(0), 3,
			...
			while (contour != null && !contour.isNull()) {
				if (contour.elem_size() > 0) {
					...
					cvDrawContours(grabbedImage, points, CvScalar.BLUE,
